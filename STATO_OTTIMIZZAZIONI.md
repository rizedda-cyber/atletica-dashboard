...
- **08/07/2026 (sera):** RISOLTO il caso "tab a tutta larghezza": nello Streamlit attuale il markup delle tab non è più `data-baseweb="tab"` ma `data-testid="stTab"` — tutte le vecchie regole pillola erano morte. Nuovo stile (pillole gialle a tutta larghezza, impilate in verticale sotto i 640px) validato dal vivo sull'app pubblicata via browser e scritto in app.py; rimosse le regole morte. Ripristino scroll verificato live: funziona.
- **08/07/2026:** verificata la corrispondenza dei modelli tra app e sprint_tools (entrambe le versioni HTML): Vittori e modello fisiologico 400m identici al millesimo. Modello 200m (SR200) allineato: l'app ora usa i 4 profili di sprint_tools (1.065/1.080/1.095/1.110, aggiunto "Resistenza Base") e le stesse soglie (<1.07/≤1.085/≤1.10/oltre). Rimossa dall'app l'opzione 250m del calcolatore Vittori (era senza formula). Auto-calibrazione da DB intatta. Nota: sprint_tools resta il riferimento per i modelli.
