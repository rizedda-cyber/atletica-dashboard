        ...
        if tipo_form == "🏃 Pista (corsa)":
            ...
            with st.form("form_corsa", clear_on_submit=True):
                ...
                for i in range(1, 13):
                    ...
                    if dist_sel == "Altro":
                        st.markdown("📝 **Inserisci distanza personalizzata:**")
                        dist_custom = st.number_input(f"Distanza (m) - Prova {i}:", min_value=1, max_value=10000, value=180, step=1, key=f"dist_custom_{i}", label_visibility="collapsed")
