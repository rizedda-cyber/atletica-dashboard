        ...
        if tipo_form == "🏃 Pista (corsa)":
            ...
            with st.form("form_corsa", clear_on_submit=True):
                ...
                for i in range(1, 13):
                    ...
                    if dist_sel == "Altro":
                        c1.markdown("📝 **Personalizzata (m):**")
                        dist_custom = c1.number_input(f"Distanza - Prova {i}:", min_value=1, max_value=10000, value=180, step=1, key=f"dist_custom_{i}", label_visibility="collapsed")
                        dist_i = f"{int(dist_custom)}m"
                    else:
                        dist_i = dist_sel
    ...
    with tab3:
        ...
        if len(short_dists) >= 2 and len(long_dists) >= 1:
            ...
            if len(df_model) >= 5:
                ...
                st.warning("Almeno 5 corrispondenze necessarie (stessa data + atleta su tutte le